...
public class GraphMLProcessor
{
	...
	/** BluePrints Graph and GraphMLReader
	 * 
	 */
	private Graph graph = new TinkerGraph();
	private GraphMLReader reader = new GraphMLReader(graph);
	...
	public Node setUniqueIdBasedOnNameAndType(String nameValue, String typeValue, String optionalParamValue, String uniqueId) 
	{
		...
		try 
		{
			for(int i = 0; i < nodes.getLength(); i++) 
			{
				...
				{
					Element data = (Element) datakeyList.item(j);
					/*if(typeValue.equalsIgnoreCase(TYPE_METHOD))	
					{
						if(data.getAttributes().item(0).toString().equals(PARAM_DATA_KEY_VALUE)
								&& datakeyList.item(j).getTextContent().toString().equals(optionalParamValue))
						{
							paramMatch = true;
						}
					}*/
	...
	/**
	 * Method returning a list of edges from a GraphML file
	 * Uses BluePrints
	 *
	 */
	public ArrayList<Edge> getEdgesFromGraphML()
	{
	...
	/** Return number of nodes using Blueprints API
	 * @param graphmlInputFilePath
	 * The file path of the graphml file
	 */
	public int getNumberOfGraphVertices(String graphmlInputFilePath)
	{
		Graph graph = new TinkerGraph();
		GraphMLReader reader = new GraphMLReader(graph);
		int counter = 0;
		try
		{
			InputStream is = new BufferedInputStream(new FileInputStream(graphmlInputFilePath));
			reader.inputGraph(is);
